...
describe('Authentication Tests', async () => {
	...
	describe('GET /api/v1/auth/verify-email', async () => {
		...
		test('should verify email successfully', async () => {
			const response = await api
				.get(
					`/api/v1/auth/verify-email?verificationToken=${admin.verificationToken}&email=${admin.email}`
				)
		...
		test('should fail with invalid verification token', async () => {
			const response = await api
				.get(
					`/api/v1/auth/verify-email?verificationToken=invalidtoken&email=${admin.email}`
				)
				.set('User-Agent', 'test-agent')
				.set('X-Forwarded-For', '127.0.0.1')
				.expect(401);
		...
		test('should fail with non-existent email', async () => {
			const response = await api
				.get(
					`/api/v1/auth/verify-email?verificationToken=${admin.verificationToken}&email=nonexistent@example.com`
				)
				.set('User-Agent', 'test-agent')
				.set('X-Forwarded-For', '127.0.0.1')
				.expect(401);
	...
	describe('POST /api/v1/auth/login', async () => {
		...
		test('should not login with incorrect password', async () => {
			...
			assert.match(
				response.body.msg,
				/Please provide a correct email and password!/i
			);
		});
